...
struct CodesignCheck {
    ...
    public static func codeSigningSubjectSummary(forPID pid: pid_t) throws -> String? {
        let certs = try codeSigningCertificates(forPID: pid)
        guard let first = certs.first else { return nil }
        return SecCertificateCopySubjectSummary(first) as String?
    }

    public static func codeSigningSubjectSummaryForSelf() throws -> String? {
        let certs = try codeSigningCertificatesForSelf()
        guard let first = certs.first else { return nil }
        return SecCertificateCopySubjectSummary(first) as String?
    }
